...
:::lemma_ "lem:leq1" (lean := "GlobalTheorem.rotation_third_partials_bounded") (parent := "global_derivative_bounds")
...
\qquad\text{and}\qquad
\left|\frac{\partial^3 f}{\partial x_i \partial x_j \partial x_k}(x_1,x_2,x_3)\right|\leq 1.
`
:::
...
:::proof "lem:leq1"
The second-partial bound is polyhedron.without.rupert, Lemma 19. The
...
:::lemma_ "lem:n2" (lean := "GlobalTheorem.bounded_partials_control_difference2") (parent := "global_derivative_bounds")
...
If
$`|\partial_{x_i}\partial_{x_j}\partial_{x_k}f(v)| \leq 1`
...
$`|g'''(c)| \leq (\sum_i \varepsilon_i)^3` using the third-partial bound.
:::
...
polyhedron.without.rupert, Theorem 17: the exact second partials at the center
pose are charged with per-axis weights, and only the cubic Lagrange remainders
are bounded via {bpref "lem:leq1"}[].
:::
